...
check_format<-function(x, correct=FALSE)
{
        ...
        # If character string and none are missing
            ...
            if (any(only2 & !notnumber)){
                if(correct==TRUE){
                    ...
                    x[only2, timecol] <- paste0(x[[timecol]][only2], "00")
                }
            }
            
            # Check for 3 characters - for AM times
            only3 <- nchar(x[[timecol]]) == 3
          
            if (any(only3 & !notnumber)){
                if(correct==TRUE){
                    print(paste("Only 3 characters in ", timecol, " column in rows: ", paste(which(only3),collapse=", "), sep=""))
                    #print(paste("ATT: ", timecol, " needs 4 characters: HHMM. Only 2 found.", sep=""))
                    print("Assuming these are times before noon. Adding 0 as first hour ciffer")
                    x[only3, timecol] <-  paste0("0", x[[timecol]][only3])
                }
            }
    ...
    gear_codes <- fecR2.0::gear_codes # lazy loading
    ...
    fishing_areas <- fecR2.0::fishing_areas
    ...
    ices_rectangles <- fecR2.0::ices_rectangles
